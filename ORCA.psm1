...
function Load-LanguageResources {
    ...
    $langFilePath = Join-Path -Path $PSScriptRoot -ChildPath "Langues\$Language.json"
    if (Test-Path $langFilePath) {
        $null = Get-Content -Raw -Path $langFilePath | ConvertFrom-Json
    } else {
        throw "Language file for '$Language' not found."
    }
}
...
function Get-LocalizedString {
    ...
    return $global:LangResources.$Key
}
...
function Get-CM365-Report {
    ...
    Load-LanguageResources -Language $Language

    try { wget https://aka.ms/orca-execution -Method head | % { $_.StatusCode } }catch {}
    ...
    $Result = Invoke-ORCA -Connect $Connect -SCC $SCC -PerformVersionCheck $PerformVersionCheck -AlternateDNS $AlternateDNS -Collection $Collection -ExchangeEnvironmentName $ExchangeEnvironmentName -Output $Output -DelegatedOrganization $DelegatedOrganization -ShowSurvey $ShowSurvey -OutputOptions $OutputOptions -OutPathFile $OutPathFile
    Write-Host "$(Get-Date) $(Get-LocalizedString -Key 'CompleteMessage') Output is in $($Result.Result)"

    # Pre-requisite checks
    if(!(Get-Command Resolve-DnsName -ErrorAction:SilentlyContinue))
    {
        Write-Warning "$(Get-LocalizedString -Key 'DnsNameWarning')"
    }

    # Generate localized HTML report
    $htmlContent = Get-Content -Path $Result.Result -Raw
    $htmlContent = $htmlContent -replace "{{Title}}", (Get-LocalizedString -Key 'Title')
    $htmlContent = $htmlContent -replace "{{Subtitle}}", (Get-LocalizedString -Key 'Subtitle')
    $htmlContent = $htmlContent -replace "{{Summary}}", (Get-LocalizedString -Key 'Summary')
    $htmlContent = $htmlContent -replace "{{Footer}}", (Get-LocalizedString -Key 'Footer')
	Return $htmlContent
...
function CountORCAStat {
    ...
    try {
        $Command = Get-Command Get-CM365-Report
